...
.DESCRIPTION
Mirrors scripts/check_build_env.sh (same options and exit codes: 0 ready; 1 unavailable
tool/daemon/builder; 2 no amd64; 64 invalid arguments). Windows-only extra: -ContainerEngine.
Requires PowerShell 7.4+ and a running Docker or Podman engine. This command is read-only.
#>
[CmdletBinding()]
param(
  [string]$Builder,
...
if ($Builder) { Fail 64 '-Builder is supported only with -ContainerEngine Docker.' }
